...
class LightGCN(nn.Module):
    ...
    def forward(self, batch=None, topk=None):
        user = batch['users']
...
#                              range(neg_items_emb_.shape[1]), indices, :]
    ...
    def negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item, k=None):
        if(k==None): k=1
        batch_size = user.shape[0]
        s_e, p_e = user_gcn_emb[user], item_gcn_emb[pos_item]  # [batch_size, n_hops+1, channel]
        if self.pool != 'concat':
        ...
        scores = (s_e.unsqueeze(dim=1) * n_e_).sum(dim=-1)  # [batch_size, n_negs, n_hops+1]
        #indices = torch.max(scores, dim=1)[1].detach()
        topk = torch.topk(scores, dim=1, k=k)[1].detach() # 5331 new code: use topk instead
        topk = topk.permute([1,0,2]) # 5331 new code: use topk instead
        
        neg_items_emb_ = n_e_.permute([0, 2, 1, 3])  # [batch_size, n_hops+1, n_negs, channel]

        # 5331 new code: use topk instead
        ls = [] 
        #for i in range(k): # 5331 new code: use topk instead
        ls.append(neg_items_emb_[[[i] for i in range(batch_size)], range(neg_items_emb_.shape[1]), topk[-1], :]) 
        # 5331 new code: use topk instead
            
        # [batch_size, n_hops+1, channel]
        # return neg_items_emb_[[[i] for i in range(batch_size)], range(neg_items_emb_.shape[1]), indices, :]
        return torch.mean(torch.stack(ls), dim=0) # 5331 new code: use topk instead
